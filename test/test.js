...
describe('Compendio Vicorum', function () {
  ...
  it('should retrieve all the information of Bari', function (done) {
    collection.findOne({ nome: 'Bari' }, function (err, item) {
      ...
      const encodedActualAltitudine = Buffer.from(item.altitudine).toString('base64')
      const encodedExpectedAltitudine = Buffer.from('5 m s.l.m.').toString('base64')
      console.log('actual:' + encodedActualAltitudine)
      console.log('expected:' + encodedExpectedAltitudine)
